...
def plot_scenario_reward(episode_num=None, start_pos=None, target_pos=None, 
    ...
    Args:
        ...
        world_size: 世界大小
    """
    # 创建环境（支持多无人机，默认使用1架无人机用于单场景分析）
    num_drones = 1  # 单场景分析使用1架无人机
    if isinstance(start_pos, list) and len(start_pos) > 0 and isinstance(start_pos[0], (list, np.ndarray)):
        # 如果start_pos是列表的列表，说明是多无人机
        num_drones = len(start_pos)
        env = MultiDronePathPlanningEnv(
            num_drones=num_drones,
            world_size=world_size,
            max_steps=max_steps,
            fixed_start_positions=start_pos,
            fixed_target_positions=target_pos
        )
    else:
        # 单无人机情况（向后兼容）
        env = MultiDronePathPlanningEnv(
            ...
            world_size=world_size,
            max_steps=max_steps,
            fixed_start_positions=[start_pos] if start_pos is not None else None,
            fixed_target_positions=[target_pos] if target_pos is not None else None
        )
    
    # 如果提供了模型路径，加载模型
    agent = None
    if model_path and os.path.exists(model_path):
        state_dim = env.observation_space.shape[1]  # Per drone
        action_dim = env.action_space.shape[1]  # Per drone
