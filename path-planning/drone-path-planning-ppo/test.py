...
def test(model_path, num_episodes=10, render=True):
    ...
    # By not specifying fixed_start_pos and fixed_target_pos, 
    # the environment will use random positions for each episode
    env = DronePathPlanningEnv(
        world_size=1000, 
        max_steps=60,
        fixed_start_pos=None,  # Use random start position
        fixed_target_pos=None  # Use random target position
    )
