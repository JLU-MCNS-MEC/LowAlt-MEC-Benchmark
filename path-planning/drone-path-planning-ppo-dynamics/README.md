# Drone Path Planning with PPO (Dynamics Model)

一个基于PPO（Proximal Policy Optimization）算法和动力学模型的无人机路径规划强化学习系统。

> **注意**: 本项目是 `Test-Project` 下的一个子工程。完整的项目结构请查看父目录的 `README.md`。
> 
> **与 `drone-path-planning-ppo` 的区别**: 本工程使用动力学模型控制无人机，动作空间为 [thrust, roll_torque, pitch_torque, yaw_torque]（4维），而不是直接的速度控制 [vx, vy]（2维）。

## 项目概述

一个面向低空边缘计算场景的模块化 benchmark 与测试平台，用于无人机路径规划任务的强化学习训练与评估。

本项目实现了一个基于PPO算法和动力学模型的无人机路径规划系统。系统会随机生成目标点，无人机需要通过控制推力、滚转力矩和俯仰力矩来规划路径到达目标点。

### 主要特性

- ✅ 基于PPO算法的强化学习实现（支持连续动作空间）
- ✅ **动力学模型控制**：使用推力、滚转力矩、俯仰力矩控制，而非直接速度控制
...
### 观测空间（Observation Space）- 17维

观测空间包含完整的动力学状态信息，帮助Agent理解当前状态并做出决策：
...
### 动作空间（Action Space）- 连续4维

动作空间使用动力学控制，Agent直接控制无人机的推力和力矩：
